...
async function GetRecommendationsExploreSection(
  ...
  try {
    ...
    WITH user, COLLECT({user: otherUser, mutuals: COUNT((user)-[:FRIENDS_WITH]-(otherUser))})[$offset_SchoolUsers..$limit_SchoolUsers] AS PeopleInSameSchool
    ...
    WITH user, PeopleInSameSchool, contacts, COLLECT({user: friendsOfFriends, mutuals: COUNT((user)-[:FRIENDS_WITH]-(friendsOfFriends))})[$offset_FriendsOfFriends..$limit_FriendsOfFriends] AS FriendsOfFriends
