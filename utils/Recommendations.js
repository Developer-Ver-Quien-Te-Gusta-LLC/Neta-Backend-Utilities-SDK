...
async function GetRecommendationsExploreSection(
  ...
  try {
    ...
    WITH user, otherUser, COUNT((user)-[:FRIENDS_WITH]-(otherUser)) AS mutuals
    WITH user, COLLECT({user: otherUser, mutuals: mutuals})[$offset_SchoolUsers..$limit_SchoolUsers] AS PeopleInSameSchool
    ...
    WITH user, friendsOfFriends, COUNT((user)-[:FRIENDS_WITH]-(friendsOfFriends)) AS mutuals
    WITH user, PeopleInSameSchool, contacts, COLLECT({user: friendsOfFriends, mutuals: mutuals})[$offset_FriendsOfFriends..$limit_FriendsOfFriends] AS FriendsOfFriends
